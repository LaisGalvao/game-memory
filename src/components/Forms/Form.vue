...
<b-form @submit="onSubmit" 
    @reset="onReset" ref="form">
    <b-col v-for="item in items" 
    :key="item.key">
      <b-form-group>
        <b-form-input
           v-if="
            ['text', 'number', 'email'].includes(item.type)"
   ...
   <div>
    ...
    >
     ...
     <b-link class="form-link-danger">
       ...
       Voltar</router-link></b-link>
    <div id="hide">State: <strong>{{ status }}</strong></div>
  </div>
</div>
   ...
   data(){
     return{
       ...
       items:[
          ...
          { key: "repetir senha", type: "password", placeholder: "Confirmar senha:"},
